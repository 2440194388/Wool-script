...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    if (process.env.csxdrhd && process.env.csxdrhd.indexOf('\n') > -1) {
      csxdrhdArr.push(process.env.csxdrhd.split('\n'))
    } else {
      csxdrhdArr.push(process.env.csxdrhd.split())
    };
    if (process.env.csxdrbody && process.env.csxdrbody.indexOf('\n') > -1) {
      csxdrbodyArr.push(process.env.csxdrbody.split('\n'))
    }  else if(process.env.csxdrbody !== undefined) {
      csxdrbodyArr.push(process.env.csxdrbody.split())
    };
    if (process.env.csxdrspbody && process.env.csxdrspbody.indexOf('\n') > -1) {
      csxdrspbodyArr.push(process.env.csxdrspbody.split('\n'))
    } else if(process.env.csxdrspbody !== undefined) {
      csxdrspbodyArr.push(process.env.csxdrspbody.split())
    };
    ...
    console.log(`------------- 共${csxdrhdArr.length}个账号-------------\n`)
      for (let i = 0; i < csxdrhdArr.length; i++) {
        if (csxdrhdArr[i]) {
          csxdrhd = String(csxdrhdArr[i]);
          csxdrbody = String(csxdrbodyArr[i]);
          csxdrspbody = String(csxdrspbodyArr[i]);
          $.index = i + 1;
          console.log(`\n开始【超市小达人${$.index}】`)
